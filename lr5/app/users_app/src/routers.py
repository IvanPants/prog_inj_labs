...
async def post_random_users(count: int, db: Session = Depends(get_db)):
    ...
    except HTTPException(status_code=400):
...
async def create_user(request: UserSchema, db: Session = Depends(get_db)):
    # try:
    find = crud.get_user_by_login(db, login=request.login)
    print(find)
    if find.login == request.login:
        return HTTPException(status_code=400, detail="user with this username already exists")
    _user = crud.add_new_user(db, user=request)
    return {
...
async def get_user_by_first_name(request: str, db: Session = Depends(get_db)):
    try:
        ...
        t1 = time.time()
        if redis_search:
            delta_t1 = t1 - start_time
            print(f"redis work")
            return {
                "message": f"user '{redis_search}' found in Redis",
                "search_time": f"time {delta_t1}",
                "status_code": "200"
            }
        start_time = time.time()
        ...
        delta_t2 = t2 - start_time
        return {
            "message"    : f"user '{_user.first_name}' found in postgres",
            "user"       : _user,
            "search-time": f"time {delta_t2}",
            "status_code": "200"
        }
    except HTTPException(status_code=400):
        return
...
async def get_all(start: int, end: int, db: Session = Depends(get_db)):
    try:
        ...
        if redis_search:
            ...
            return {"search": f"redis_search: {redis_search}",
                    "search_time": f"time: {delta_t1}"}
        ...
        return {
            "message"    : f"users found from postgres",
            "searh_time" : f"time: {delta_t2}",
            "users"      : _users,
            "status_code": "200"
        }
    except HTTPException(status_code=400):
...
async def update_user(user_id: int, new_first_name: str, new_second_name: str,
    ...
    try:
        ...
        find = crud.get_user_by_login(db, login=new_login)
        if find:
